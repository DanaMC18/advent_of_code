...
def part_1() -> int:
    """Find number of assignements that fully overlap."""
    input = _load_data()
    overlaps = 0
...
def _load_data() -> List[List]:
    ...
    # test = [[[int(i) for i in sect.split('-')] for sect in pair] for pair in pairs]
    # # return [item for sublist in test for item in sublist]
    # return test
    return pairs
