...
const plotWire = wirePath => {
  let path  = [];
  let start = [0,0];
  let subPath;

  for (loc of wirePath) {
    subPath = pathFinder(start, loc);
    path.push(...subPath);
    start = subPath[subPath.length - 1];
  }
  return path;
}

const pathFinder = (start, loc) => {
  const dir     = loc[0];
  const dist    = parseInt(loc.substring(1));
  const subPath = [];
  let x         = start[0];
  let y         = start[1];
...
// two identical arrays aren't "equal" in js
const isInArray = (val, arr) => {
  for (el of arr) {
    if (val[0] === el[0] && val[1] === el[1]) return true;
  }

  return false;
}

const path1         = plotWire(WIRE_1);
const path2         = plotWire(WIRE_2);
const intersections = path1.filter(plot => isInArray(plot, path2));
