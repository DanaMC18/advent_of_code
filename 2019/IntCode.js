...
const replace   = (val, pos, arr) => arr[pos] = val;
...
const position  = (pos, arr) => arr[arr[pos]];
const immediate = (pos, arr) => arr[pos];
...
const paramMode = (iCode, opStr, index) => {
  ...
  for (let i = 2; i < 4; i++) {
    index += 1
    mode  = modes[i] || 0;
    param = PARAM_MODES[mode].call(null, index, iCode);
    params.push(param);
  }
...
const diagnostics = (intCode, input, pointer = 0) => {
  ...
  if (String(opCode).length > 1) {
    [opCode, params] = paramMode(intCode, String(opCode), pointer);
    params.push(intCode[pointer + 3]);
  }
  ...
  switch (opCode) {
    ...
    case 3:
      ...
      func.call(null, input, position, intCode);
      pointer += 2;
      break;
    case 4:
  ...
  diagnostics(intCode, input, pointer);
}

diagnostics(INTCODE, 5);
