...
const path1 = plotWire(WIRE_1).map(p => p.join(','));
const path2 = plotWire(WIRE_2).map(p => p.join(','));

const fewestSteps = () => {
  let min;

  for (cross of INTERSECTIONS) {
    i1 = path1.indexOf(cross.join(',')) + 1;
    i2 = path2.indexOf(cross.join(',')) + 1;
    let sum = i1 + i2;
